...
Feature: Publish messages receivecd from multiple subscribers at a high rate
  ...
  Scenario: Publish messages from multiple publishers to a single subscriber
    ...
    When 100 publishers send the message "message" to Lance Broker 10 times
    Then the subscriber with name "subscriberName" receives the message "message" 1000 times in 4000 mSeconds
