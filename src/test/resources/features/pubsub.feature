Feature: Publish and Subscribe Feature
  ...
  @Standard
  Scenario Outline: Subscriber receives message
    Given Lance Broker has a subscriber port of 5556 and a message port of 5555
    ...
    Examples: One subscriber one message
      | topicName | subscriberName | message        |
      | "topic1"  | "subName1"     | "test message" |

  @Standard
  Scenario Outline: Lance takes the latest subscriber by name not port
    Given Lance Broker has a subscriber port of 5556 and a message port of 5555
    And Lance Broker can receive 1 message
    ...
    Examples: Subscribe and unsubscribe from one topic
      | topicName | subscriberName |
      | "topic1"  | "subName"      |

  @Standard
  Scenario Outline: Multiple subscribers receive their respective messages
    Given Lance Broker has a subscriber port of 5556 and a message port of 5555
    ...
    Examples: Two subscribers receive their respective messages
  ...
  @Standard
  Scenario Outline: A subscriber can receive multiple messages
    Given Lance Broker has a subscriber port of 5556 and a message port of 5555
    ...
    Examples: One subscriber receives multiple messages
      | topicName | subscriberName | message        |
      | "topic1"  | "subName1"     | "test message" |

  @Standard
  Scenario Outline: Multiple subscribers receive same messages when subscribe to same topic
    Given Lance Broker has a subscriber port of 5556 and a message port of 5555
    And Lance Broker can receive 10 messages
    And Lance Broker is receiving 2 subscriptions
    And a subscriber is created with listening port 3333 with name <subscriberName1>
    ...
    And the subscriber with name <subscriberName2> receives the message <message1> 10 times

    Examples: Two subscribers receive their respective messages
      | topicName1 | subscriberName1 | subscriberName2 | message1         |
      | "topic1"   | "subName1"      | "subName2"      |"test1 message1"  |
